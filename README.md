...
#USER & MINERAL MANAGEMENT

@app.route('/add_user', methods=['POST'])
def add_user():
    if session.get('role') not in ['Administrator', 'Developer']:
        return redirect(url_for('login'))
    username = request.form['username']
    password = request.form['password']
    role = request.form['role']
...
@app.route('/delete_mineral/<int:id>', methods=['POST'])
def delete_mineral(id):
    if session.get('role') not in ['Administrator', 'Developer']:
        return redirect(url_for('login'))
    m = Mineral.query.get(id)
    if m:
        db.session.delete(m)
        db.session.commit()
    return redirect(url_for('dashboard'))
...
def add_country():
    if session.get('role') not in ['Administrator', 'Developer']:
        return redirect(url_for('dashboard'))  # Block other roles
    country = request.form['country']
    gdp = request.form['gdp']
    projects = request.form['key_projects']
    notes = request.form['notes']
    if CountryProfile.query.filter_by(country=country).first():
        return "Country already exists!"
    db.session.add(CountryProfile(country=country, gdp=float(gdp) if gdp else None,
                                  key_projects=projects, notes=notes))
    db.session.commit()
    return redirect(url_for('dashboard'))


@app.route('/delete_country/<int:id>', methods=['POST'])
def delete_country(id):
    if session.get('role') not in ['Administrator', 'Developer']:
        return redirect(url_for('dashboard'))  # Block other roles
    c = CountryProfile.query.get(id)
    if c:
        db.session.delete(c)
        db.session.commit()
    return redirect(url_for('dashboard'))

# MAP PAGE with filer option.
@app.route("/map")
def map_page():
    ...
    if mineral:
        query = query.filter_by(mineral=mineral)
    ...
    for rec in records:
        folium.Marker(
            [rec.lat, rec.lon],
            popup=f"<b>{rec.country}</b><br>{rec.mineral}: {rec.quantity} tonnes",
            tooltip=f"{rec.country} - {rec.mineral}",
        ).add_to(m)

    path = os.path.join(app.root_path, "map.html")
    m.save(path)
...
def charts_page():
    ...
    data = [{"Country": m.country, "Mineral": m.mineral, "Quantity": m.quantity} for m in query.all()]
    if not data:
        return "<h5 class='text-center text-muted mt-3'>No data available for the selected filters.</h5>"
    df = pd.DataFrame(data)
    fig = px.bar(df, x="Country", y="Quantity", color="Mineral", title="Mineral Production by Country")
